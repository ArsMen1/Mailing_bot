...
def current_group_detailing_nps_message(info):
    ...
    res = tabulate(table, headers=["Группа", "NPS"])

    return "`" + res + "`"


def semester_detailing_indicators_message(sem: str, nps: str, retirement: str, redflags: str) -> str:
    result = f"🗓 *Семестр: {sem}*\n\n"
    if nps:
        result += f"🫂 NPS: {nps}\n"
    if retirement:
        result += f"🏃 Выбываемость: {retirement}\n"
    if redflags:
        result += f"Редфлаги: {redflags}"
